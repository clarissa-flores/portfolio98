...
var menus = {
	"&Game": [
		...
		// },
		{
			label: "De&ck...",
			action: ()=> {
				if (window.showDeckChooser) {
					window.showDeckChooser();
				}
			},
		...
		MENU_DIVIDER,
		{
			...
			action: ()=> {
				try {
					if (parent && parent.__osOpenMap && parent.__osOpenMap.solitaire) {
						const win = parent.__osOpenMap.solitaire;
					...
					// ignore cross-frame errors and fall through
				}
				close();
			},
